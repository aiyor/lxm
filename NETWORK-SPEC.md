...
- **OVN DNS Exfiltration Prevention on `internet: false` (R8 / EC-18)**: The provider daemon baseline installs an OpenFlow allow (priority 200) for port 53 to upstream resolvers. On `internet: false` OVN networks, LXM installs priority 400 reject rules on port 53 UDP/TCP to external/uplink resolvers (`<resolver>/32`), strictly overriding the daemon baseline and preventing DNS leakage on isolated networks. In-network resolvers inside `S_V` are preserved and remain accessible via G0 (`allow S_V → S_V`) for intra-subnet name resolution.
