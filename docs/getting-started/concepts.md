...
Between your declaration and your live LXD host sits one guardrail: every change is previewed before it is made.
...
You always run `plan` first. Before anything touches LXD, you see exactly what would change and can stop if the plan does not match your intent. A `plan` that surprises you is a free opportunity to fix the manifest — no harm done.
...
* A teammate can apply the same manifests to their own LXD host and get the same result — reproducible fleets, not one-off setups.

## What happens when you run lxm

In broad terms, every `plan`/`apply` pass does the same thing:

1. Read every manifest in the target directory and merge each container's file with the shared base.
2. Fetch the current state of your LXD host.
...
That is the whole model: **declare, preview, apply.** Everything else in lxm — mounts, networking, provisioning scripts, snapshots — is a way to say more about the end state you want.
