...
// NewUnixDriver connects to a local LXD UNIX socket.
...
// NewRemoteDriver connects to a remote LXD HTTPS endpoint.
func NewRemoteDriver(url string, args *lxd_client.ConnectionArgs) (*Driver, error) {
	client, err := lxd_client.ConnectLXD(url, args)
	if err != nil {
		return nil, err
	}
	return NewDriver(client), nil
...
func (d *Driver) ListInstances(ctx context.Context) ([]provider.Instance, error) {
	fullInstances, err := d.client.GetInstancesFull(lxd_client.GetInstancesFullArgs{InstanceType: api.InstanceTypeAny})
	if err != nil {
		return nil, err
	}
...
func (d *Driver) UpdateInstance(ctx context.Context, name string, req provider.InstanceUpdateRequest, etag string) error {
	...
	op, err := d.client.UpdateInstance(name, lxdPut, etag)
	if err != nil {
		return err
	}
	return common.WaitOpContext(ctx, op)
...
func (d *Driver) DeleteInstance(ctx context.Context, name string) error {
	op, err := d.client.DeleteInstance(name, false)
	if err != nil {
		return err
	}
	return common.WaitOpContext(ctx, op)
...
func (d *Driver) RebuildInstance(ctx context.Context, name string, req provider.InstanceRebuildRequest) error {
	op, err := d.client.RebuildInstance(name, api.InstanceRebuildPost{
		Source: api.InstanceSource{
			Type:        api.SourceType(req.Source.Type),
...
func (d *Driver) CreateNetwork(ctx context.Context, net provider.NetworkCreateRequest) error {
	...
	for time.Now().Before(deadline) {
		...
		if getErr == nil && n != nil && n.Status != "Pending" {
			if net.Type == "ovn" {
	...
	if n, _, getErr := d.clusterClient().GetNetwork(net.Name); getErr == nil && n != nil && n.Status == "Pending" {
...
func (d *Driver) CreateStoragePoolVolume(ctx context.Context, pool string, vol provider.StorageVolumeCreateRequest) error {
	op, err := d.client.CreateStoragePoolVolume(pool, api.StorageVolumesPost{
		Name:        vol.Name,
		Type:        vol.Type,
...
func (d *Driver) UpdateStoragePoolVolume(ctx context.Context, pool, volType, name string, vol provider.StorageVolumeUpdateRequest, etag string) error {
	op, err := d.client.UpdateStoragePoolVolume(pool, volType, name, api.StorageVolumePut{
		Description: vol.Description,
		Config:      vol.Config,
	}, etag)
	if err != nil {
...
func (d *Driver) DeleteStoragePoolVolume(ctx context.Context, pool, volType, name string) error {
	op, err := d.client.DeleteStoragePoolVolume(pool, volType, name)
	if err != nil {
		return err
	}
	return common.WaitOpContext(ctx, op)
...
func (d *Driver) CreateInstanceFile(ctx context.Context, name string, path string, content io.Reader, mode int, uid, gid int64) error {
	...
	args := lxd_client.InstanceFileArgs{
		Content:   readSeeker,
		UID:       uid,
		GID:       gid,
		Mode:      mode,
...
func toLXDRules(rules []provider.NetworkACLRule) []api.NetworkACLRule {
