...
// clusterNetworkMock is a minimal in-memory LXD daemon for exercising the
...
func newLXDClusterTestDriver(t *testing.T, srv *httptest.Server) *lxd.Driver {
	t.Helper()
	d, err := lxd.NewRemoteDriver(srv.URL, &lxd_client.ConnectionArgs{InsecureSkipVerify: true})
	if err != nil {
...
func TestLXDDriver_CreateNetwork_ClusterStaging(t *testing.T) {
	ctx := context.Background()
