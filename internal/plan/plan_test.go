...
func TestReconciler_NilManifest_Error(t *testing.T) {
	...
	_, err := r.Compute(nil, nil, nil, nil, config.BuiltinImageRemotes(), false)
	if err == nil {
...
func TestReconciler_Compute_AbsentStatus(t *testing.T) {
	...
	p2, err := rec.Compute(conf, live, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReconciler_Compute_NetworkConfig_EmittedAndDiffed(t *testing.T) {
	...
	p3, err := rec.Compute(conf, liveMatch, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	p4, err := rec.Compute(confNoNet, liveStale, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReconciler_Compute_Recreate_ImageChange(t *testing.T) {
	...
	p2, _ := rec.Compute(conf, live, nil, nil, config.BuiltinImageRemotes(), true)
...
func TestReconciler_Compute_RecreatePowerTransitions(t *testing.T) {
	...
	pRun, err := rec.Compute(confRunning, liveRunning, nil, nil, config.BuiltinImageRemotes(), false)
	...
	pStop, err := rec.Compute(confStopped, liveStopped, nil, nil, config.BuiltinImageRemotes(), false)
...
func TestReconciler_Compute_AliasImage_ReplanNoop(t *testing.T) {
	...
	p2, err := rec.Compute(conf, liveAlpine, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReconciler_Compute_FingerprintImage_ReplanNoop(t *testing.T) {
	...
	pPrefix, err := rec.Compute(confPrefix, live, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReconciler_Compute_CloudInitError_ReturnsError(t *testing.T) {
	...
	_, err = rec.Compute(confUpdate, live, nil, nil, config.BuiltinImageRemotes(), false)
	if err == nil {
...
func TestReconciler_Compute_AllSummaryActions(t *testing.T) {
	...
	p1, _ := rec.Compute(confRecreate, liveRecreate, nil, nil, config.BuiltinImageRemotes(), false)
	...
	p2, _ := rec.Compute(confDelete, liveRecreate, nil, nil, config.BuiltinImageRemotes(), false)
	...
	p3, _ := rec.Compute(confStart, liveStopped, nil, nil, config.BuiltinImageRemotes(), false)
...
func TestReconciler_Compute_RecreateBuildPayloadError(t *testing.T) {
	...
	_, err := rec.Compute(conf, live, nil, nil, config.BuiltinImageRemotes(), false)
	if err == nil {
...
func TestReconciler_Compute_UpdateWithPowerStateTransition(t *testing.T) {
	...
	pStart, err := rec.Compute(confStart, liveStopped, nil, nil, config.BuiltinImageRemotes(), false)
...
func TestReconciler_NilManifestAndRebuildExtension(t *testing.T) {
	...
	// Nil manifest error
	if _, err := rec.Compute(nil, nil, nil, nil, config.BuiltinImageRemotes(), false); err == nil {
	...
	// hasRebuildExt = true
	pExt, err := rec.Compute(confRecreate, liveSnap, nil, nil, config.BuiltinImageRemotes(), true)
	if err != nil {
	...
	// hasRebuildExt = false
	pFallback, err := rec.Compute(confRecreate, liveSnap, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("Compute failed: %v", err)
	}
	...
	pDesc, err := rec.Compute(confRecreate, liveDesc, nil, nil, config.BuiltinImageRemotes(), true)
	if err != nil {
		t.Fatalf("Compute failed: %v", err)
	}
	...
	pYAMLRecipe, err := rec.Compute(confYAMLRecipe, nil, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("Compute failed: %v", err)
	}
	...
	pStop, err := rec.Compute(confStop, liveRunning, nil, nil, config.BuiltinImageRemotes(), false)
	...
	if _, err := rec.Compute(confBadCloudInit, nil, nil, nil, config.BuiltinImageRemotes(), false); err == nil {
	...
	pFull, err := rec.Compute(confFullProps, nil, nil, nil, config.BuiltinImageRemotes(), false)
	if err != nil {
		t.Fatalf("Compute failed: %v", err)
	}
...
func TestReconciler_Compute_TypeChange_ForcesRebuildFallback(t *testing.T) {
	...
	p, err := rec.Compute(conf, live, nil, nil, config.BuiltinImageRemotes(), true) // hasRebuildExt = true
	if err != nil {
		t.Fatalf("Compute failed: %v", err)
	}
